...
public class AmazoniteArmorHelmet extends ArmorItem {
    ...
    public void onArmorTick(ItemStack itemstack, World world, PlayerEntity player) {
        if (player.getItemBySlot(EquipmentSlotType.HEAD).getItem() == ModItems.AMAZONITE_HELMET.get()) {
            player.addEffect(new EffectInstance(Effects.INVISIBILITY, 300, 0, false, false, true));
        }
    }

    @OnlyIn(Dist.CLIENT)
    public void appendHoverText(ItemStack pStack, @Nullable World pWorld, List<ITextComponent> pTooltip, ITooltipFlag pFlag) {
        pTooltip.add(new TranslationTextComponent(this.getDescriptionId() + ".desc").withStyle(TextFormatting.BLUE));
    }
